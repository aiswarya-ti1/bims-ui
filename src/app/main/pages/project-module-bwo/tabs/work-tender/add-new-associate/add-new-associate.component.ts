...
User_ID:number=0;
  ...
  constructor(public dialogRef_AddNew: MatDialogRef<AddNewServiceAssocComponent>, @Inject(MAT_DIALOG_DATA) private data: any,
...
addAssociate(values)
{
  this.projectService.addAssoc(values).subscribe(result=>{console.log(result);
...
getOneAssociate()
{
  this.certfService.getOneAssociate(this.Assoc_ID,1).subscribe(result=>{console.log(result);
    this.details=result;
