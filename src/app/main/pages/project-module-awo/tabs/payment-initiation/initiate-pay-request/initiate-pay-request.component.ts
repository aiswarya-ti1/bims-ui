...
User_Name: string='';
  ...
  {
    console.log($values);
    this.projectService.initiatePayment($values).subscribe(result=>{console.log(result);
      if(result['Error']==true)
      {
        alert('Can`t initiate!! Enter amount less than '+result['Amount']);
      }
  ...
  getOneInitiatePay($id)
  {
    this.projectService.getOneInitiatePay($id).subscribe(result=>{
      ...
      this.transactionForm.controls['type'].setValue(result[0]['Trans_Type']);
